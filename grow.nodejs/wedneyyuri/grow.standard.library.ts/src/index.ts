...
const dataGrowthKey = (d: DataGrowth) =>
  `${d.Country}${d.Indicator}${d.Year}`.toUpperCase();
...
const GetStatus: RequestListener = (req, res) => {
  if (!("BRZNGDP_R2002" in mapGrow)) {
    res.writeHead(400, JSON.stringify({ msg: "not finished" }), {
...
const RoutePut: RequestListener = (req, res) => {
  ...
  req.on("end", () => {
    ...
    mapGrow[key] = input;
    res.writeHead(200, JSON.stringify({ msg: "ok" }), {
      "Content-Type": "application/json",
    });
    res.end();
  });
...
const RouteDelete: RequestListener = (req, res) => {
  ...
  delete mapGrow[key];
  res.writeHead(200, JSON.stringify({ msg: "deleted" }), {
    "Content-Type": "application/json",
  });
  res.end();
};
...
console.log(
  "\x1b[36m%s\x1b[0m",
  `Running on http://0.0.0.0:${serverPort} (Press CTRL+C to quit)`
);
